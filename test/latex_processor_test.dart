  ...
  group('LaTeXProcessor', () {
    group('LaTeX Detection', () {
      test('should detect inline LaTeX expressions', () {
        ...
        expect(LaTeXProcessor.containsInlineLaTeX('Hello \$x = 5\$ world'),
            isTrue);
        expect(LaTeXProcessor.containsBlockLaTeX('Hello \$x = 5\$ world'),
            isFalse);
      });

      test('should detect block LaTeX expressions', () {
        expect(
            LaTeXProcessor.containsLaTeX('Hello \$\$x = 5\$\$ world'), isTrue);
        expect(LaTeXProcessor.containsBlockLaTeX('Hello \$\$x = 5\$\$ world'),
            isTrue);
        expect(LaTeXProcessor.containsInlineLaTeX('Hello \$\$x = 5\$\$ world'),
            isFalse);
    ...
    group('LaTeX Expression Counting', () {
      test('should count LaTeX expressions correctly', () {
        ...
        expect(LaTeXProcessor.countLatexExpressions('One \$x = 5\$ expression'),
            equals(1));
        expect(
            LaTeXProcessor.countLatexExpressions(
                'Two \$x = 5\$ and \$y = 10\$ expressions'),
            equals(2));
        expect(
            LaTeXProcessor.countLatexExpressions(
                'Block \$\$z = 15\$\$ expression'),
            equals(1));
        expect(
            LaTeXProcessor.countLatexExpressions(
                'Mixed \$a = b\$ and \$\$c = d\$\$ expressions'),
            equals(2));
      });
    });
    ...
    group('LaTeX Expression Extraction', () {
      test('should extract LaTeX expressions', () {
        const text =
            'Formula \$E = mc^2\$ and matrix \$\$\\begin{matrix} a & b \\\\ c & d \\end{matrix}\$\$';
        ...
        expect(expressions[1],
            equals('\$\$\\begin{matrix} a & b \\\\ c & d \\end{matrix}\$\$'));
      });
    });
    ...
    group('Text Segmentation', () {
      ...
      test('should parse text with block LaTeX', () {
        const text =
            'Matrix:\$\$\\begin{matrix} a & b \\\\ c & d \\end{matrix}\$\$End';
        final segments = LaTeXProcessor.parseTextSegments(text);
        expect(segments, hasLength(3));
        ...
        expect(segments[1].content,
            equals('\\begin{matrix} a & b \\\\ c & d \\end{matrix}'));
        expect(segments[1].type, equals(SegmentType.blockLaTeX));
        expect(segments[1].isLaTeX, isTrue);
        expect(segments[1].fullExpression,
            equals('\$\$\\begin{matrix} a & b \\\\ c & d \\end{matrix}\$\$'));
    ...
    group('Position Checking', () {
      test('should correctly identify positions within LaTeX', () {
        ...
        expect(
            LaTeXProcessor.isPositionInLatex(text, 5), isFalse); // Before LaTeX
        expect(LaTeXProcessor.isPositionInLatex(text, 7),
            isTrue); // Inside inline LaTeX
        expect(LaTeXProcessor.isPositionInLatex(text, 9),
            isTrue); // Inside inline LaTeX
        expect(LaTeXProcessor.isPositionInLatex(text, 13),
            isFalse); // After inline LaTeX
        expect(LaTeXProcessor.isPositionInLatex(text, 25),
            isTrue); // Inside block LaTeX
        expect(LaTeXProcessor.isPositionInLatex(text, 35),
            isFalse); // After block LaTeX
      });
    ...
    group('Edge Cases', () {
      ...
      test('should handle LaTeX with special characters', () {
        ...
        expect(segments[0].content,
            equals('\\alpha + \\beta = \\gamma \\cdot \\delta'));
      });
    });
  });
}
